...
declare global {
	interface Window {
		...
		sendTransactionToMetaMask?: (txData: TransactionData) => Promise<string>;
	}
}
...
const ChatView: React.FC = () => {
	...
	const processTransactionFromMessage = useCallback((message: ChatMessage) => {
		// This function sends a transaction to MetaMask using the already connected address
		const sendToMetaMask = (transactionData: TransactionData): Promise<string> => {
			return new Promise((resolve, reject) => {
				// Check if MetaMask is available
				if (!window.ethereum) {
					reject(new Error('MetaMask is not installed. Please install MetaMask to continue.'));
					return;
				}
				
				// Check if we have an address
				if (!address) {
					reject(new Error('No wallet address found. Please connect your wallet.'));
					return;
				}
				...
				const mainTxParams = {
					...
					value: transactionData.value === '0' ? '0x0' : transactionData.value,
				};
		...
		if (transaction) {
			...
			window.sendTransactionToMetaMask = (transData: TransactionData) => {
				return sendToMetaMask(transData);
			};

			// Just log the transaction data to console for debugging
			console.log('Found transaction data, preparing to send to MetaMask:', txData);
	...
	useEffect(() => {
		if (messages.length > 0) {
			...
			if (lastMessage.role === 'assistant' && lastMessage.id && !processedMessageIds.current.has(lastMessage.id)) {
				// Mark this message as processed
				processedMessageIds.current.add(lastMessage.id);
				
				// Process the transaction and automatically send to MetaMask if transaction is found
				const hasTransaction = processTransactionFromMessage(lastMessage);
				
				// If transaction was found, automatically send it to MetaMask
				if (hasTransaction && window.latestTransaction && window.sendTransactionToMetaMask) {
					window.sendTransactionToMetaMask(window.latestTransaction)
						.then((hash) => {
